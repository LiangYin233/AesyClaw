...
export class CommandManager implements CommandCatalog {
    ...
    createScope(
        ...
        return {
            ...
            registerMany: (commands) =>
                commands.map((command) => this.register(owner, command, options)),
    ...
    getCommand(name: string): CommandDefinition | undefined {
        ...
        return (
            this.commands.get(resolvedName)?.command ?? this.resolveNamespacedCommandBySuffix(name)
        );
    }
    ...
    hasCommand(name: string): boolean {
        return (
            this.aliasMap.has(name.toLowerCase()) ||
            this.commands.has(name) ||
            Boolean(this.resolveNamespacedCommandBySuffix(name))
        );
    }
