...
export class SpeechToTextTool implements Tool {
    ...
    readonly description =
        '【重要】将语音文件转换为文字。当用户发送语音消息时使用此工具。参数：audio_path（必填，语音文件的绝对路径或URL）。注意：此工具每个语音只能调用一次。返回转录后的文本内容。示例：audio_path="G:\\Project\\.aesyclaw\\media\\audio_xxx.mp3"';
    ...
    ): ProviderRuntimeConfig | ToolExecutionResult {
    ...
    private async resolveAudioInput(
        ...
        { fileBuffer: ArrayBuffer; mimeType: string; fileName: string } | ToolExecutionResult
    > {
    ...
    async execute(args: unknown, _context: ToolExecuteContext): Promise<ToolExecutionResult> {
        ...
        try {
            ...
            const transcription = await client.audio.transcriptions.create({
                file: new File([audioInput.fileBuffer], audioInput.fileName, {
            ...
            logger.info(
                { audioPath: audio_path, textLength: transcription.text.length },
                '语音转文字完成',
            );
...
export class ImageUnderstandingTool implements Tool {
    ...
    readonly description =
        '【重要】理解图片内容。当用户发送图片时使用此工具。参数：image_path（必填，图片的绝对路径或URL），prompt（必填，你想了解图片的什么问题）。注意：此工具每个图片只能调用一次，多次调用同一图片会浪费资源。调用后返回图片理解结果。示例：image_path="G:\\Project\\.aesyclaw\\media\\image_xxx.jpg"，prompt="详细描述这张图片的内容"';
    ...
    private async resolveImageUrl(
        imagePath: string,
    ): Promise<{ imageUrl: string } | ToolExecutionResult> {
...
export class SendMsgTool implements Tool {
    ...
    readonly description =
        '立即额外发送一条消息到当前会话，可包含文字和媒体文件。适用于先发送补充说明、阶段性结果、图片、音频、视频或文件。注意：这会立刻发送，不会替代最终回复。';
    ...
    async execute(args: unknown, context: ToolExecuteContext): Promise<ToolExecutionResult> {
        ...
        try {
            ...
            const errorMessage = toErrorMessage(error);
            logger.error({ chatId: context.chatId, error: errorMessage }, '发送额外消息失败');
            return {
                success: false,
                content: '',
                error: errorMessage,
