...
export class BuiltInToolExecutor {
  static async executeCalculator(
    args: { expression: string },
    _context: ToolExecuteContext
  ): Promise<ToolExecutionResult> {
    try {
      const expression = args.expression
        .replace(/sqrt\((\d+)\)/gi, 'Math.sqrt($1)')
        .replace(/\^/g, '**');
  ...
  static async executeCurrentTime(
  ...
  static async executeWebSearch(
    ...
    logger.info({ query: args.query, traceId: context.traceId }, '🔍 执行 Web 搜索（模拟）');

    await new Promise(resolve => setTimeout(resolve, 500));

    const maxResults = args.maxResults || 3;
    const mockResults = [
      `结果 1: 关于 "${args.query}" 的第一个搜索结果...`,
      `结果 2: 关于 "${args.query}" 的第二个搜索结果...`,
      `结果 3: 关于 "${args.query}" 的第三个搜索结果...`,
