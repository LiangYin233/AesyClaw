...
function resolveSessionForReceive(chatService: ChatService, ctx: ChannelContext): ResolvedSessionState {
...
export function getSessionForCommandContext(chatService: ChatService, ctx: CommandContext): ChatContext | null {
  return chatService.getForCommand(ctx);
}

export function switchRoleForCommandContext(chatService: ChatService, ctx: CommandContext, roleId: string): { success: boolean; message: string } {
  return chatService.switchRole(ctx, roleId);
}

export function getRoleInfoForCommandContext(chatService: ChatService, ctx: CommandContext): {
  roleId: string;
  roleName: string;
  allowedTools: string[];
} {
...
export async function compactSessionForCommandContext(chatService: ChatService, ctx: CommandContext): Promise<{ success: boolean; message: string }> {
