...
export function buildAesyiuEngine(options: BuildAesyiuEngineOptions): {
    ...
    const context = new AgentContext({
        provider,
        modelId: options.llmConfig.model,
    });
    ...
    engine.useLLM(
        createHookAwareLLMMiddleware(options.stats, options.hookRuntime, hookTools, hookSkills),
    );
    engine.useTool(
        createHookAwareToolMiddleware(toolIndex, options.stats, options.hookRuntime, {
            chatId: options.chatId,
