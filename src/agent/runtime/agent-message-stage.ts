...
export const agentStage: MiddlewareFunc = async (
    ctx: ChannelContext,
    next: () => Promise<void>,
) => {
    ...
    if (!sessionContext) {
        logger.error(
            {},
            'SessionContext not found, ensure session message stage is registered before agent stage',
        );
    ...
    try {
        ...
        if (result.success) {
            ...
            logger.error(
                { sessionId, chatId: ctx.received.chatId, error: result.error },
        ...
        logger.error(
            { sessionId, chatId: ctx.received.chatId, error: errorMessage },
            'Agent exception',
        );
    }
