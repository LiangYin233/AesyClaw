  ...
  const response = await completeSimple(
    cacheModel,
    {
      systemPrompt: [
        ...
        '',
 'Goal',
 '- The user\'s objective or task (e.g. "构建一个用户登录系统")',
        '',
        'Constraints',
 '- Constraints and preferences from the user (e.g. "使用 TypeScript，不要用任何框架")',
        '',
        'Progress',
 '- Completed / in-progress / blocked tasks (e.g. "登录接口已完成，注册接口进行中")',
        '',
        'Key Decisions',
 '- Important decisions and the reasoning behind them (e.g. "选择 JWT 而非 Session，因为需要无状态")',
        '',
        'Next Steps',
 '- What remains to be done (e.g. "实现密码重置功能")',
        '',
        'Critical Context',
 '- Data, file paths, or notes needed to continue working (e.g. "主文件在 src/auth.ts")',
        '',
        'Keep each section concise. If a section has no content, omit it entirely. Do not mention that you are summarizing or refer to missing context.',
      ].join('\n'),
