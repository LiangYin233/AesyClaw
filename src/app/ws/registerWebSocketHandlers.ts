...
export function registerWebSocketHandlers(args: RegisterWebSocketHandlersArgs): () => void {
  ...
  server.registerRpc('agents.abortWorkerSession', (params) =>
    agentWorkersService.abortSession(requiredString(params, 'sessionKey'))
  );
  ...
  server.registerRpc('cron.toggle', async (params) => {
    const payload = asRecord(params);
    const result = await requireService(cronApiService, 'Cron service').toggleJob(requiredString(payload, 'id'), parseToggleCronJob(payload).enabled);
    server.publish('cron.list');
    return result;
  });
  ...
  server.registerSubscription('sessions.detail', {
    getSnapshot: (params) => getSessionDetailSnapshot(sessionService, requiredString(params, 'key'))
  });
  ...
  if (skillManager) cleanups.push(skillManager.onChange(() => { server.publish('skills.list'); server.publish('skills.detail'); }));
  ...
  cleanups.push(eventBus.on('mcp.tools.synced', () => { for (const t of ['system.status', 'system.tools', 'mcp.list', 'mcp.detail']) server.publish(t); }));
  cleanups.push(eventBus.on('plugin.runtime.updated', () => { for (const t of ['system.status', 'system.tools', 'plugins.list']) server.publish(t); }));
