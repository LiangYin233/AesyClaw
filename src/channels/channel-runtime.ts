...
export class ChannelRuntime {
    ...
    private async loadChannelPlugins(channels: Record<string, unknown>): Promise<void> {
        ...
        logger.info(
            { loadedChannels: this.deps.channelManager.getChannelCount() },
    ...
    private async loadChannelPluginEntry(
        ...
        try {
            ...
            if (!loaded.entryPath || !loaded.module) {
                logger.warn(
                    { pluginName, candidates: loaded.candidates },
                    'Channel plugin entry point not found',
                );
                return;
            }
            ...
            const channelConfig =
                (channels[channelPlugin.name] as Record<string, unknown> | undefined) || {};
            const registered = await this.deps.channelManager.registerChannel(
                channelPlugin,
                channelConfig,
            );
            if (registered) {
                logger.info(
                    { channelName: channelPlugin.name },
                    `${channelPlugin.name} channel plugin loaded`,
                );
            }
        } catch (error) {
            ...
            logger.error(
                { error: errorMessage, stack: errorStack, pluginName },
