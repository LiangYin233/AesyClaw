...
export function createCommandMiddleware(commandCatalog: CommandCatalog): MiddlewareFunc {
    return async (ctx: ChannelContext, next: () => Promise<void>): Promise<void> => {
        ...
        if (!command) {
            ...
            logger.warn({ commandName: parsed.name, chatId: ctx.received.chatId }, '未知命令');
            return;
        }
        ...
        try {
            ...
            logger.info({ commandName: command.name, success: result.success }, '命令执行完成');
        } catch (error) {
            logger.error({ commandName: command.name, error }, '命令执行失败');
