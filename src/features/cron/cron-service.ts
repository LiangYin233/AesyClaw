...
import { getNextFutureRunAt, normalizeScheduleInput } from '@/platform/cron/schedule-engine.js';
...
import type { CreateCronJobInput, CronExecutor } from './types.js';
...
export class CronService {
    ...
    async stop(): Promise<void> {
        ...
        if (pending.length > 0) {
            logger.info(
                { activeJobCount: pending.length },
                'Waiting for active cron jobs to complete',
            );
    ...
    private advanceSchedule(job: CronJob, referenceTime: Date, startedAt: string): void {
        ...
        if (nextRunAt === null) {
            logger.info(
                ...
                'Cron job reached terminal schedule state',
            );
        }
    }
