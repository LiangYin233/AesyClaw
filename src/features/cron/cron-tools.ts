...
const CreateCronSchema = z.object({
  ...
  prompt: z.string().trim().min(1, 'prompt 不能为空').describe('到点后交给 agent 执行的提示词'),
  schedule_type: z.enum(['once', 'daily', 'interval']).describe('调度类型：once、daily 或 interval'),
  run_at: z.string().trim().min(1).optional().describe('仅 once 可用。未来时间的 ISO 时间字符串，例如 2026-04-20T15:30:00+08:00'),
  delay_minutes: z.number().int().positive().optional().describe('仅 once 可用。多少分钟后执行，传入后会立即转换为一次性任务'),
  daily_time: z.string().trim().regex(/^\d{2}:\d{2}$/).optional().describe('仅 daily 可用。系统本地时区的 HH:MM，例如 09:30'),
  interval_minutes: z.number().int().positive().optional().describe('仅 interval 可用。按固定节拍每隔多少分钟执行一次'),
}).superRefine((value, ctx) => {
  if (value.schedule_type === 'once') {
    const onceFieldCount = Number(Boolean(value.run_at)) + Number(value.delay_minutes !== undefined);
    if (onceFieldCount !== 1) {
      ctx.addIssue({
        code: 'custom',
  ...
  if (value.schedule_type === 'daily') {
    ...
    if (value.run_at !== undefined || value.delay_minutes !== undefined || value.interval_minutes !== undefined) {
      ctx.addIssue({
        code: 'custom',
  ...
  if (value.run_at !== undefined || value.delay_minutes !== undefined || value.daily_time !== undefined) {
    ctx.addIssue({
      code: 'custom',
...
class CreateCronTool implements Tool {
  ...
  readonly description = '创建一个新的调度任务。支持一次性执行、延迟执行、每日固定时间执行和固定节拍间隔执行。';
  ...
  async execute(args: unknown, _context: ToolExecuteContext): Promise<ToolExecutionResult> {
    ...
    try {
      ...
      logger.info({ jobId: job.id, scheduleType: job.schedule.type }, 'Cron job created via tool');
  ...
  private buildCreateInput(input: z.infer<typeof CreateCronSchema>) {
    ...
    return {
      name: input.name,
      prompt: input.prompt,
      schedule: { type: 'interval' as const, intervalMinutes: input.interval_minutes! },
    };
  }
}
...
class ListCronTool implements Tool {
  ...
  readonly description = '列出当前所有调度任务，包括 id、名称、调度类型、上次执行时间和下次执行时间。';
