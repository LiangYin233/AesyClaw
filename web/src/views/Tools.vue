      ...
      <table class="w-full border-collapse separate font-body text-sm">
        <thead>
          <tr>
            ...
            <th class="px-4 py-3 text-left text-mid-gray font-heading font-medium text-[0.7rem] uppercase tracking-[0.08em] bg-[#FAF8F3] sticky top-0">Description</th>
            <th class="px-4 py-3 text-left text-mid-gray font-heading font-medium text-[0.7rem] uppercase tracking-[0.08em] bg-[#FAF8F3] sticky top-0">Owner</th>
          </tr>
        </thead>
        <tbody>
          <template v-for="tool in tools" :key="tool.name">
            <tr class="cursor-pointer bg-[#FDFBF9] transition-colors duration-[0.15s] ease hover:bg-[rgba(20,20,19,0.03)]" @click="toggleTool(tool.name)">
              <td class="px-4 py-3 border-b border-[var(--color-border)]">
                <ChevronRightIcon class="w-[14px] h-[14px] text-mid-gray transition-transform duration-[0.2s] ease shrink-0" :class="{ 'rotate-90': expanded === tool.name }" />
              </td>
              <td class="px-4 py-3 border-b border-[var(--color-border)] font-heading font-medium text-dark">{{ tool.name }}</td>
              <td class="px-4 py-3 border-b border-[var(--color-border)] text-mid-gray">{{ tool.description }}</td>
              <td class="px-4 py-3 border-b border-[var(--color-border)]">
            ...
            <tr v-if="expanded === tool.name" class="bg-[rgba(20,20,19,0.02)]">
              <td colspan="4">
                <div class="p-5">
                  ...
                  <div v-if="paramFields(tool.parameters).length === 0" class="text-mid-gray text-sm italic">No parameters</div>
                  <div v-else class="flex flex-col gap-2">
                    <div v-for="p in paramFields(tool.parameters)" :key="p.name" class="flex items-baseline gap-2 text-sm">
                      <span class="font-heading font-medium text-dark">{{ p.name }}</span>
                      <span class="text-xs text-mid-gray uppercase tracking-[0.04em]">{{ p.type }}</span>
                      <span v-if="p.required" class="text-xs text-[#b85c3a] uppercase tracking-[0.04em]">required</span>
                      <span v-if="p.description" class="text-xs text-mid-gray">— {{ p.description }}</span>
                    </div>
                  </div>
                </div>
              </td>
          ...
          <tr v-if="tools.length === 0">
            <td colspan="4" class="text-mid-gray text-center py-10 font-body italic text-sm">No tools</td>
          </tr>
        </tbody>
      </table>
    </div>
