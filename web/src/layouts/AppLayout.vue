<template>
  <div class="flex flex-col h-screen overflow-hidden">
    <header
      class="h-topbar bg-[#FDFBF8] border-b border-[var(--color-border)] flex items-center justify-between px-6 shrink-0 z-10"
    >
      <div class="flex items-center gap-2">
        <button
          class="lg:hidden flex items-center justify-center w-8 h-8 rounded-sm border border-[var(--color-border)] bg-transparent text-mid-gray cursor-pointer transition-all duration-[0.15s] ease hover:bg-[#FAF7F4] hover:text-dark mr-1"
        ...
        <span
          class="font-heading text-[0.7rem] font-medium text-mid-gray bg-[#FAF7F4] px-[0.5rem] py-[0.15rem] rounded border border-[var(--color-border)]"
          >v{{ appVersion }}</span
        >
      </div>
      <div class="flex items-center gap-2">
    ...
    <div class="flex flex-1 overflow-hidden">
      <!-- Desktop sidebar -->
      <aside
        class="hidden lg:flex w-sidebar bg-[#FAF7F4] flex-col shrink-0 border-r border-[var(--color-border)]"
      >
        <nav class="flex flex-col p-3 gap-1 flex-1">
          <RouterLink
            ...
            :class="{
              '!text-dark !bg-[#F7F0EA] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:w-[3px] before:h-5 before:bg-primary before:rounded-r-[3px]':
      ...
      <Transition name="drawer">
        <div v-if="drawerOpen" class="fixed left-0 right-0 bottom-0 top-topbar z-50 lg:hidden">
          <div
            class="absolute inset-0 bg-[rgba(20,20,19,0.25)] backdrop-blur-sm"
            @click="drawerOpen = false"
          ></div>
          <aside
            class="drawer absolute left-0 top-0 bottom-0 w-sidebar bg-[#FAF7F4] flex flex-col shrink-0 border-r border-[var(--color-border)] shadow-lg"
